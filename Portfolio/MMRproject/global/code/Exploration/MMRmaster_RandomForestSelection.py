...
# In[ ]:
#Concating the data frame together using the ISO value
...
mother['EDFMLE_MLE%']
 ...
 # In[ ]:
# Defining X and y 
feature_cols = ['ABR%', 'Abortion_scale', 'AOD_FMLE', 'GNI', 'EDFMLE_MLE%', 'cell_Subscription%', 'contraceptives%', 'attend%', 'prenat%', 'homicide100K']  #'AOD_MLE', 'AOD_BTSX', 'ED_FMLE%', 'ED_MLE%', 
...
# Max_features=5 is best and n_estimators=50
rfreg = RandomForestRegressor(n_estimators=50, max_features=5, oob_score=True, random_state=1)
rfreg.fit(X, y)
...
X_important = rfreg.transform(X, threshold='median')
X_important.shape
...
rfreg = RandomForestRegressor(n_estimators=50, max_features=5, random_state=1)
