...
exports = pd.read_csv('total exports 2015 (billions$).csv')

# Change index to state 
exports = exports.set_index(['State'])
...
pieces = [USMMR, exports, narcotics, weed]
opioids = pd.concat(pieces, axis=1, join_axes=[exports.index])
...
# Defining X and y 
# not optimized
feature_cols = ['total exports', 'weed enthusiasm rank', 'MedianIncome($)', 'economic distress', 'Teen Birth Rate per 1,000', 'PPR non-white ', 'State Taxes Per Capita', 'MMR', 'Abortion_Policy_rank', 'Medicaid_extend_Pregnancy', 'PPR_White'] #, 'Pill_InsurePol', 'EC_access'

# optimized                  
#feature_cols = ['weed enthusiasm rank', 'Teen Birth Rate per 1,000', 'PPR non-white ','total exports'] #'economic distress', 'MedianIncome($)', 
...
rfreg = RandomForestRegressor(n_estimators=30, max_features=1, oob_score=True, random_state=1)
rfreg.fit(X, y)
...
rfreg = RandomForestRegressor(n_estimators=250, max_features=3, random_state=1)
...
opioids.plot(kind='scatter', x='weed enthusiasm rank', y='NormNarcDeaths',figsize=(5,5))
...
data_important = ['NormNarcDeaths','Teen Birth Rate per 1,000', 'total exports', 'weed enthusiasm rank'] #'PPR non-white ', 'State Taxes Per Capita', 'weed enthusiasm rank', 'economic distress', 'MMR', 
