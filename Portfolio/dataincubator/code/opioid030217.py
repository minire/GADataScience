...
feature_cols = ['MMR', 'total exports', 'weed enthusiasm rank', 'MedianIncome($)', 'Medicaid_extend_Pregnancy', 'economic distress', 'Teen Birth Rate per 1,000', 'PPR_White', 'PPR non-white ', 'Abortion_Policy_rank', 'State Taxes Per Capita'] #, 'Pill_InsurePol', 'EC_access'
...
for feature in feature_range:
    rfreg = RandomForestRegressor(n_estimators=250, max_features=feature, random_state=1)
...
rfreg = RandomForestRegressor(n_estimators=250, max_features=3, oob_score=True, random_state=1)
rfreg.fit(X, y)
...
sfm = SelectFromModel(rfreg, threshold=0.0789, prefit=True)
X_important = sfm.transform(X)
print(X_important.shape[0],X_important.shape[1])

# In[ ]:
# Check the RMSE for a Random Forest that only includes important features
rfreg = RandomForestRegressor(n_estimators=50, max_features=3, random_state=1)
...
# In[]:
# visualizing X and y 
data_cols =['weed enthusiasm rank','NormNarcDeaths', 'MMR', 'MedianIncome($)', 'Medicaid_extend_Pregnancy', 'economic distress', 'Teen Birth Rate per 1,000', 'PPR_White', 'PPR non-white ', 'Abortion_Policy_rank', 'Pill_InsurePol', 'EC_access', 'State Taxes Per Capita', 'total exports']
